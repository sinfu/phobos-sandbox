...
/**
Transforms a string representing declarations into a variadic template.
...
$(D meta.lambda) also supports local entity _captures and recursions.

Captures:

 The template generated by $(D meta.lambda) can use local entities
 explicitly passed via the $(D captures) parameter of $(D meta.lambda).
 Like parameters, captured entities get named $(D p)-$(D w).
--------------------
struct Example(T, UU...)
{
...
Params:
     decl = String representing a valid template body.  The body must
            declare a symbol $(D _) that represents the result of the
...
Returns:
 Variadic template whose body is $(D decl) and evaluates to $(D _) declared
 in that body.
 */
